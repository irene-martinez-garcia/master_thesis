...
```{r heatmap data}
heatmap_data <- sig_fisher_results %>%
  dplyr::select(kegg, prop_EHI, prop_GTDB)%>%
  pivot_longer(!kegg, names_to = "source", values_to= "proportion" ) %>%
   mutate(
    source = case_when(
      source == "prop_EHI"  ~ "EHI",
...
```{r plot the keggs}
ggplot(heatmap_data,
       aes(x = source, y = kegg, fill = proportion)) +
  geom_tile() +
