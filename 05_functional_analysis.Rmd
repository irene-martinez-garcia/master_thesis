...
kegg_dist <- vegdist(kegg_rel_nz, method = "bray")  # or euclidean
...
# PERMANOVA test
permanova_result <- adonis2(kegg_dist ~ source, 
...
pcoa_df <- data.frame(PC1 = pcoa_res$points[,1], PC2 = pcoa_res$points[,2], ID = rownames(kegg_rel_nz))
...
plot_data <- kegg_rel_nz_meta %>%
  select(ID, source, all_of(common_KOs)) %>%
...
direction_check <- kegg_rel_nz_meta %>%
  select(source, all_of(common_KOs)) %>%
...
```{r heatmap with all significant ko from wilcoxon}
#Prepare the matrix (KOs as rows, MAGs as columns)
heatmap_mat <- kegg_rel_nz_meta %>%
  column_to_rownames("ID") %>%
  select(all_of(wilcox_results$KO)) %>%
  as.matrix() %>%
  t()
