...
get_kegg_info <- function(ko_id) {
  ...
  safe_get <- function(id) {
    tries <- 5
    for (i in 1:tries) {
      result <- try(keggGet(id, timeout = 60), silent = TRUE)
      if (!inherits(result, "try-error")) return(result)
      Sys.sleep(1 * i)   # exponential backoff
    }
    return(NULL)  # failed after all retries
  }
  
  query <- safe_get(ko_id)
  
  if (is.null(query)) {
    return(c(
      KO = ko_id,
      Name = "Error",
      Definition = "Timeout or fetch failed",
      Pathways = "N/A"
    ))
  }
...
# V2: checks if annotation columns are empty and keeps running
...
run_fisher_tests <- function(mat, metadata, annotation_name = "feature") {
  ...
  if (is.null(mat) || !is.data.frame(mat) || nrow(mat) == 0) {
    message(sprintf("run_fisher_tests('%s'): 'mat' is NULL/empty; returning empty.", annotation_name))
    return(tibble::tibble())
  }
  if (!"genome" %in% names(mat)) {
    message(sprintf("run_fisher_tests('%s'): 'mat' lacks 'genome' column; returning empty.", annotation_name))
    return(tibble::tibble())
  }
  feature_cols <- setdiff(names(mat), "genome")
  if (length(feature_cols) == 0) {
    message(sprintf("run_fisher_tests('%s'): no feature columns (only 'genome'); returning empty.", annotation_name))
    return(tibble::tibble())
  }

  # metadata checks
  if (is.null(metadata) || !is.data.frame(metadata) || nrow(metadata) == 0) {
    message(sprintf("run_fisher_tests('%s'): 'metadata' is NULL/empty; returning empty.", annotation_name))
    return(tibble::tibble())
  }
  if (!all(c("ID_clean", "host_type") %in% names(metadata))) {
    message(sprintf("run_fisher_tests('%s'): metadata must contain 'ID_clean' and 'host_type'; returning empty.", annotation_name))
    return(tibble::tibble())
  }

  # Ensure both host groups exist (animal & human) before continuing
  host_groups <- unique(metadata$host_type)
  if (!all(c("animal", "human") %in% host_groups)) {
    message(sprintf(
      "run_fisher_tests('%s'): metadata$host_type must include both 'animal' and 'human'; returning empty.",
      annotation_name
    ))
    return(tibble::tibble())
  }
  ...
  # --- Safety guard: if nothing is present, return empty ----------------------
  if (nrow(pa_long) == 0) {
    message(sprintf("run_fisher_tests('%s'): no genomes carry any features; returning empty.", annotation_name))
    return(tibble::tibble())
  }
  # ---------------------------------------------------------------------------
  ...
  # --- Safety guard: if join produced no host_type, return empty --------------
  if (!"host_type" %in% names(pa_with_host) || all(is.na(pa_with_host$host_type))) {
    message(sprintf("run_fisher_tests('%s'): no matches between 'genome' and metadata$ID_clean; returning empty.", annotation_name))
    return(tibble::tibble())
  }
  # ---------------------------------------------------------------------------
...
<!--chapter:end:06_one_species.Rmd-->
...
Right now we dont have all the species annotated, so here is testing the loop with the ones we have annotations for;
