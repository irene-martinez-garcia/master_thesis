# Data statistics (HUMAN)

### Load data
```{r load data statistics_human}
load("data/human/data.Rdata")
```

Total number of MAGs
```{r count n MAGS_human}
genome_metadata %>%
  dplyr::count(source)
```

```{r mean completeness and contamination_human}
genome_metadata %>% 
  summarise(
...
```{r genome_quality_plot_human, message=FALSE, warning=FALSE, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
