...
- Make list will all the accession numbers of the MAGs and run this script to obtain more metadata. (modify script a bit to make it scalable).
```{bash, eval = FALSE}
...
gtdb_accession_metadata<- read_tsv("data/parabacteroides_distasonis_GTDB_accession_metadata.tsv") %>%
  mutate(cleaned_accession = str_replace(accession, "^(RS_|GB_)", ""))
```
...
gtdb_metadata <- full_join(gtdb_accession_metadata, gtdb_search_metadata,  by = c("cleaned_accession" = "Accession")
  )
