...
Download the EHI_MAG index for each species (in /data/mags_metadata folder) and the curl file and metadata tsv from the GTDB and place in each species directory in Mjolnir.
...
```{bash, eval = FALSE}
snakemake -s snakefiles/downloading_mags.smk --cores 8
```
...
### Sample metadata
```{r load gtdb metadata}
gtdb_metadata <- read_tsv("data/p_dist_gtdb_metadata.tsv")
```
```{r load EHI metadata}
...
# Prepare GTDB metadata with standardized column names
gtdb_clean <- gtdb_metadata %>%
  select(
    ID = Accession,
    species = "Organism Name",
    gtdb_taxonomy = "GTDB Taxonomy",
    sample_type = "Isolation Source",
...
#Join
combined_metadata <- bind_rows(ehi_clean, gtdb_clean)
