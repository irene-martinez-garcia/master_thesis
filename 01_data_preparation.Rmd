...
tip_data_for_plot <- tip_data_for_plot %>%
  left_join(
    tip_df %>% dplyr::select(ID, source, host_species, country, locality, continent),
    by = c("label" = "ID")
  )
...
```{r final_pangenome_fig_heatmap_tree, fig.height=8, fig.width=16, fig.fullwidth=TRUE, eval = FALSE}
hc <- as.hclust(tree)
...
```{r dendogram matrix figure, fig.height=10, fig.width=16, fig.fullwidth=TRUE, eval = FALSE}
...
```{r testing with complexheatmap to include ani values of dendogram, fig.height=10, fig.width=16, fig.fullwidth=TRUE, eval = FALSE}
