...
```{r gift plots}
coords_for_join <- coords_for_join %>% distinct(Genome, .keep_all = TRUE)
...
GIFTs_elements %>%
  ...
  left_join(coords_for_join, by = "Genome") %>%
...
```
```{r functions plot}
coords_for_join_2 <- coords_for_join %>% rename(Sample = Genome)
